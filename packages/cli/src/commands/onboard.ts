...
export function registerOnboardCommand(program: Command): void {
  program
    ...
    .action(async () => {
      ...
      console.log(chalk.hex('#FF6B35').bold('  ║') + chalk.white.bold(`   🔥 ${APP_NAME} — Setup Wizard       `) + chalk.hex('#FF6B35').bold('║'));
      console.log(chalk.hex('#FF6B35').bold('  ╚══════════════════════════════════════╝'));
      console.log('');
      console.log(chalk.gray('  Este wizard vai configurar o ForgeAI passo a passo.'));
      console.log(chalk.gray('  Você pode executar novamente a qualquer momento com: ') + chalk.cyan('forge onboard'));
      console.log('');
      ...
      if (!hasJwtSecret || !hasVaultPassword) {
        console.log(chalk.yellow.bold('  ⚡ Passo 1: Segurança'));
        console.log(chalk.gray('  Gerando chaves de segurança automaticamente...\n'));
        ...
        console.log(chalk.green('  ✓ JWT_SECRET gerado'));
        console.log(chalk.green('  ✓ VAULT_MASTER_PASSWORD gerado'));
        console.log(chalk.gray(`  Salvo em: ${envPath}\n`));
      } else {
        console.log(chalk.green.bold('  ✓ Passo 1: Segurança') + chalk.gray(' — chaves já configuradas\n'));
      }

      // ─── Step 2: LLM Provider ────────────────────────────
      console.log(chalk.yellow.bold('  ⚡ Passo 2: Provedor de IA'));
      ...
      const providerAnswer = await inquirer.prompt([
        {
          ...
          choices: [
            { name: '🟣 Anthropic (Claude) — recomendado', value: 'anthropic' },
            { name: '🟢 OpenAI (GPT-4o)', value: 'openai' },
            { name: '🔵 Google (Gemini)', value: 'google' },
            { name: '🌙 Moonshot (Kimi K2.5)', value: 'moonshot' },
            { name: '🔷 DeepSeek', value: 'deepseek' },
            { name: '⚡ Groq (Llama)', value: 'groq' },
            { name: '🟠 Mistral', value: 'mistral' },
            { name: '✖️ xAI (Grok)', value: 'xai' },
            { name: '⏩ Pular (configurar depois na Dashboard)', value: 'skip' },
          ],
        },
      ]);
      ...
      if (providerAnswer.provider !== 'skip') {
        ...
        console.log(chalk.green(`\n  ✓ API Key do ${pName} salva`));
        console.log(chalk.gray('  A key será armazenada no Vault criptografado ao iniciar o gateway.\n'));
      } else {
        console.log(chalk.gray('\n  Você pode configurar providers depois em: ') + chalk.cyan('http://localhost:18800/settings\n'));
      }

      // ─── Step 3: Channels ────────────────────────────
      console.log(chalk.yellow.bold('  ⚡ Passo 3: Canais de comunicação'));
      ...
      const channelAnswer = await inquirer.prompt([
        {
          ...
          choices: [
            ...
            { name: '🌐 WebChat (sempre ativo)', value: 'webchat', checked: true, disabled: 'incluso' },
          ],
        },
      ]);
      ...
      if (selectedChannels.includes('whatsapp')) {
        console.log(chalk.green('\n  ✓ WhatsApp será configurado via QR code ao iniciar o gateway.\n'));
      }
      ...
      if (Object.keys(channelTokens).length > 0) {
        ...
        console.log(chalk.green(`\n  ✓ ${Object.keys(channelTokens).length} token(s) salvo(s) no .env`));
      }

      // ─── Step 4: Agent Personality ────────────────────────────
      console.log(chalk.yellow.bold('\n  ⚡ Passo 4: Personalidade do agente'));
      ...
      const personalityAnswer = await inquirer.prompt([
        ...
        },
        {
          type: 'editor',
          name: 'persona',
          message: 'Descrição da personalidade (abra o editor ou pressione Enter para usar o padrão):',
          default: '',
        },
      ]);
      ...
      if (personalityAnswer.agentName !== 'ForgeAI' || personalityAnswer.language !== 'auto') {
        const langLabel: Record<string, string> = { 'pt-BR': 'Português (BR)', 'en': 'English', 'es': 'Español', 'auto': 'Auto-detect' };
...
`;
        ...
        console.log(chalk.green(`  ✓ IDENTITY.md salvo`));
      }
      ...
      if (personalityAnswer.persona && personalityAnswer.persona.trim().length > 10) {
        ...
        console.log(chalk.green(`  ✓ SOUL.md salvo`));
      }

      // ─── Step 5: Provider key → save for startup ────────────────────────────
      if (apiKeyForProvider && providerAnswer.provider !== 'skip') {
        ...
        console.log(chalk.green(`  ✓ Provider config salvo (será ativado no primeiro start)`));
      }
      ...
      try {
        ...
        spinner.warn('MySQL não disponível — o gateway funcionará sem persistência SQL (usando JSON files)');
      }
      ...
      console.log('');
      console.log(chalk.hex('#FF6B35').bold('  ╔══════════════════════════════════════╗'));
      console.log(chalk.hex('#FF6B35').bold('  ║') + chalk.white.bold('   ✅ Setup completo!                 ') + chalk.hex('#FF6B35').bold('║'));
      console.log(chalk.hex('#FF6B35').bold('  ╚══════════════════════════════════════╝'));
      console.log('');
      console.log(chalk.white('  Resumo:'));
      console.log(chalk.gray('  ├─ Segurança:  ') + chalk.green('JWT + Vault configurados'));
      console.log(chalk.gray('  ├─ Provider:   ') + (providerAnswer.provider !== 'skip' ? chalk.green(providerAnswer.provider) : chalk.yellow('configurar depois')));
      console.log(chalk.gray('  ├─ Canais:     ') + chalk.green(selectedChannels.length > 0 ? selectedChannels.join(', ') : 'WebChat (padrão)'));
      console.log(chalk.gray('  ├─ Agente:     ') + chalk.green(personalityAnswer.agentName));
      console.log(chalk.gray('  └─ Idioma:     ') + chalk.green(personalityAnswer.language));
      console.log('');
      ...
      console.log(chalk.cyan('  3. ') + chalk.gray('Verificar saúde:          ') + chalk.cyan.bold('forge doctor'));
